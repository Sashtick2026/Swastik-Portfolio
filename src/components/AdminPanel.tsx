...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "hero" | "projects" | "about" | "testimonials" | "contact" | "versions" | "messages">("dashboard");
    ...
    performSave(async () => {
      if (editingTestimonialId === "new") {
        const newTesti: Review = {
          ...
          avatar: testimonialForm.avatar || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop&crop=face",
          isFeatured: true
        };
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <aside className="w-[80px] md:w-[240px] border-r border-white/5 bg-[#08080a]/40 flex flex-col justify-between py-6 select-none flex-shrink-0">
                <div className="space-y-2 px-3">
                  {[
                    { id: "dashboard", label: "DASHBOARD", icon: <Laptop className="w-5 h-5" /> },
                    { id: "messages", label: "INBOX / MESSAGES", icon: <Inbox className="w-5 h-5" /> },
                    { id: "hero", label: "HERO SECTION", icon: <FileText className="w-5 h-5" /> },
                    { id: "projects", label: "PROJECTS", icon: <Briefcase className="w-5 h-5" /> },
                    { id: "about", label: "ABOUT SECTION", icon: <User className="w-5 h-5" /> },
                    ...
                    { id: "versions", label: "VERSION LOG", icon: <History className="w-5 h-5" /> },
                  ].map((tab) => (
                    <button
                      key={tab.id}
                    ...
                    {editingTestimonialId && (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                          ...
                          <div>
                            <label className="block font-mono text-[10px] tracking-wider text-white/40 uppercase mb-2">Avatar URL (Unsplash/Cloudinary)</label>
                            <input
                              type="text"
